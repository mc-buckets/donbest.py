...
class BaseDonbestResponse(object):
	...
	INT_FIELDS  = ["game_number", "rot", "seating_capacity", 
				...
				"away_under_price", "home_under_price"
	]
	...
	def _setattr_to_attributes(self, element):
		tag = element.tag
		attribs = element.attrib
		setattr(self, tag, attribs)
...
class League(BaseDonbestResponse):
	...
	def get_sport(self):
		league_id = self.id
		s = self._donbest.league(id=league_id)
		sport = s.sport
		return sport


##TODO: Change to be a generic participant class from which Team will inherit
			...
			for p in participants:
				if p.find(".//team") is not None:
					...
					if "side" in p.attrib.keys():
						side = p.attrib["side"]
					t = p.find(".//team")
					team = Team.from_participant_node(node=t, rotation=rot, side=side, donbest=e)
					parts.append(team)
...
class Score(BaseDonbestResponse):
	"""Tests an API call to get info about the score of a game"""
...
class Donbest(object):
	...
	def __call__(self, *args, **kwargs):
		if self.endpoint is None:
			...
			#default to parsing the response to native python objects
			...
			if parse_response:
				...
				if self.endpoint in ["schedule", "current_schedule", "schedule_inplay"]:
					...
					for s in node.findall(".//sport"):
						...
						for l in s.findall(".//league"):
							league = League.from_xml_collection(l, sport=sport, donbest=self)
							for g in l.findall(".//group"):
								group = Group(g, donbest=self)
								for e in l.findall(".//event"):
									if self.endpoint == "schedule_inplay":
										event = Event.from_inplay_xml_collection(e, league=league, group=group, donbest=self)
									else:
										event = Event.from_full_xml_collection(e, league=league, group=group, donbest=self)
				...
				if self.endpoint in ["odds", "open", "close"] and "league_id" in kwargs:
					lines = []
					for e in node.findall(".//event"):
						event = Event(node=e, donbest=self)
						for l in e.findall(".//line"):
							line = Line.from_xml_collection(node=l, event=event, donbest=self)
							lines.append(line)
					if len(lines) == 0:
						raise EmptyResponseError(
							"The response from the API came back empty."
				...
				if self.endpoint == "location":
					...
					for l in node.findall(".//location"):
						...
						location = Location.from_xml_collection(l, city=city, donbest=self)
				...
				if self.endpoint == "team":
					...
					if 'id' in kwargs:
						...
						team = Team(node.find(".//team"), donbest=self)
						team.set_league(league)
						return team
					else:
						for s in node.findall(".//sport"):
							sport = Sport(s, donbest=self)
							for l in s.findall(".//league"):
								...
								for t in teams:
									team = Team.from_xml_collection(t, league=league, donbest=self)
